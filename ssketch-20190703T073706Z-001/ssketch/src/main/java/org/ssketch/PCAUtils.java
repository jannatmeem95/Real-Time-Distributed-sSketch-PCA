...
//import FileFormat.OutputFormat;
...
class PCAUtils {
	...
	///change by Farzana
	static double[] dense_mllib_VectorTimesMatrix(org.apache.spark.mllib.linalg.Vector vector, Matrix matrix,
												  double[] arr) {
		int nRows = matrix.numRows();
		int nCols = matrix.numCols();
		for (int c = 0; c < nCols; c++) {
			double dotres = 0;
			for (int r = 0; r < nRows; r++)
				dotres += vector.apply(r) * matrix.getQuick(r, c);
			arr[c] = dotres;
		}
		return arr;
	}
	...
	public static void outerProductWithoutIndices(org.apache.spark.mllib.linalg.Vector yi, Vector ym, double[] xi,
												  Vector xm, double[][] resArray) {
		// 1. Sum(Yi' x (Xi-Xm))
		...
		for(i=0; i < yi.size(); i++)
		{
			...
			for (int xCol = 0; xCol < xSize; xCol++) {
				double centeredValue = xi[xCol] - xm.getQuick(xCol);
				resArray[i][xCol] += centeredValue * yScale;
			}
	 ...
	 */
    /*public static void printMatrixToFile(org.apache.spark.mllib.linalg.Matrix m, FileFormat.OutputFormat format, String outputPath) {
	...
	public static void printMatrixInDenseTextFormat(org.apache.spark.mllib.linalg.Matrix m) {
		String outputPath="~\\Desktop\\Densetext.txt";
		try {
	...
	public static void printPCAtoFile(double[] S,int clusters, int nPCs, int subSample, org.apache.mahout.math.Matrix eigen_vector, String outputPath, int blkSize, int stride)
	{
		try {
			String fileLocation = outputPath+ File.separator +"pca.txt";
			...
			out.append("Singular Values:\n");
			String SingularVal=null;
			for(int i=0;i<S.length-1;i++)
			{
				SingularVal+=S[i]+",";
			}
			SingularVal+=S[S.length-1]+"\n";

			String str="BLockSize: "+blkSize+" , Stride: "+stride+ ", clusters:" +clusters+
					", nPCs: "+nPCs + ", subSample: "+subSample +"\n\n";
			out.append(str);

			out.append(SingularVal);
			out.append("\n\n");

			int row=eigen_vector.numRows();
			int cols=eigen_vector.numCols();

			String SingularMat=null;

			for(int i=0;i<row;i++)
			{
				for (int j=0;j<cols;j++)
				{
					SingularMat+=eigen_vector.getQuick(i,j)+"  ";
				}
				SingularMat+="\n";
			}

			out.append(SingularMat);
			out.append("******\n\n\n\n");
			out.close();

		}
		catch (Exception e) {
			System.out.println("NO pcaFile");
			Log.error("Output file " + outputPath + " not found ");
		}
	}


	public static void printMatrix(org.apache.mahout.math.Matrix mat ,int clusters, int nPCs, int subSample, String outputPath, int blkSize, int stride) {
		try {
			String fileLocation = outputPath + File.separator + "outputPrincipal.csv";
			File f = new File(fileLocation);
			PrintWriter out = null;
			if (f.exists() && !f.isDirectory()) {
				out = new PrintWriter(new FileOutputStream(f, true));
	...
	public static void printStatToFile_small(ArrayList<Stat> stat_list,int clusters, int nPCs, int subSample, String outputPath, int blkSize, int stride){
		try {

			String fileLocation = outputPath + File.separator + "stat.txt";
			File f = new File(fileLocation);

			String tmp = "BLockSize: " + blkSize + " , Stride: " + stride + ", clusters:" + clusters +
					", nPCs: " + nPCs + ", subSample: " + subSample +"clusters: "+clusters+"\n\n";
			...
			double meanSpectralError = 0;
			String s="statSize "+stat_list.size();
			out.append(s);

			for(int w=0;w<stat_list.size();w++) {

				Stat stat = stat_list.get(w);

				System.out.println("in blkNumber ");
				System.out.println(stat.blockNum);
				meanRuntime += stat.totalRunTime;
				meanSpectralError += stat.spectral_error;

				out = new PrintWriter(new FileOutputStream(f, true));
				String str="Block Number: "+stat.blockNum+"\n\n";
				String changedist="**Changed_dist Rows**";
				for(int i=0;i<stat.changeDist.size();i++){
					changedist+=stat.changeDist.get(i)+", ";
				}
				out.append(changedist);
				out.append("\n"+"******\n\n\n");
				out.close();
			}
			...
			double meanRuntime_forBlockSize=meanRuntime/stat_list.size();
			meanSpectralError/=stat_list.size();

			out = new PrintWriter(new FileOutputStream(f, true));
			String meanRunTime="meanRuntime for this BlockSize "+meanRuntime_forBlockSize;
			String meanSpectral="mean Spectral error for this BlockSize "+meanSpectralError;
			...
			System.out.println("NO StatFile");
			Log.error("Output file " + outputPath + " not found ");
		}
	}


	public static void printStatToFile(ArrayList<Stat> stat_list,int clusters, int nPCs, int subSample, String outputPath, int blkSize, int stride) {
		try {
			...
			for(int w=0;w<stat_list.size();w++){
				...
				String spec="\nSpectral error: "+stat.spectral_error+"\n";
				out.append(spec);
